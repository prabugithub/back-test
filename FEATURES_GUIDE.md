...
- `maxConsecutiveHighBreaksAtEntry` / `maxConsecutiveLowBreaksAtEntry` — longest run of consecutive bars that each broke the prior bar's high **without** breaking its low (mirror for low-breaks) — the Brooks "impulse micro-channel" test, e.g. a clean 4-bar breakout. Unlike Break Count (total breaks, possibly scattered), this demands an unbroken streak; outside and inside bars reset the run. Searched over the **Consecutive** lookback (Session Settings, default 10) ending at the frozen leg extreme (entry bar in `PIVOT` mode). Gates via `consecutiveBreakFilter` (`none`/`min` "Streak Required"/`max` "Streak Capped") + `consecutiveBreakThreshold` (default `4`), direction-aligned. Off by default on all regimes and presets.
- `legStartIndexAtEntry` / `legEndIndexAtEntry` / `legBarCountAtEntry` — the bar-index bounds (and inclusive length) of the frozen impulse leg whose extreme anchored the strength-metric windows. `undefined` means the entry was graded with entry-bar windows instead (manual trades or `PIVOT` mode).

  **Frozen impulse-leg anchor for pullback entries:** the trend-strength filters — Efficiency Ratio, Bar Overlap, Break Count, Consecutive Breaks, Bar Range (incl. dominance), EMA21/EMA50 Slope, and EMA20 Gap-Bar — are meant to grade the impulse leg that led into the trade, not the pullback the entry bar sits in. For `H_SIGNAL`/`CONFLUENCE` entries (Al Brooks H/L pullback signals) their windows **end at the frozen leg extreme** — the swing high/low where the pullback began — with **lengths always taken from the Instrumentation Lookbacks in Session Settings** (window length is never derived from the leg's own bar count; real legs are often 1–3 bars, which would make the metrics degenerate and the lookback settings inert). The anchor is **frozen when the pullback starts**: H1, H2, H3… of the same pullback structure all grade the same window, and it only moves (metrics re-evaluate) when price breaks beyond the original swing extreme — the next leg then starts at the pullback's own deepest extreme. This holds even if the H/L count resets on a minor push high/low inside the pullback. For `PIVOT`-mode entries all windows end at the entry bar (no pullback concept applies). EMA20 Bias (always-in direction), ATR Depth, and the pivot-sequence filters always measure at the entry bar, since they describe current context rather than leg strength.
