...
- `maxConsecutiveHighBreaksAtEntry` / `maxConsecutiveLowBreaksAtEntry` — longest run of consecutive bars that each broke the prior bar's high **without** breaking its low (mirror for low-breaks) — the Brooks "impulse micro-channel" test, e.g. a clean 4-bar breakout. Unlike Break Count (total breaks, possibly scattered), this demands an unbroken streak; outside and inside bars reset the run. Measured within the frozen impulse leg (see below); in `PIVOT` mode, over the **Breaks** lookback window. Gates via `consecutiveBreakFilter` (`none`/`min` "Streak Required"/`max` "Streak Capped") + `consecutiveBreakThreshold` (default `4`), direction-aligned. Off by default on all regimes and presets.
- `legStartIndexAtEntry` / `legEndIndexAtEntry` / `legBarCountAtEntry` — the exact bar-index bounds (and inclusive length) of the frozen impulse leg the strength metrics were computed over. `undefined` means the entry was graded with entry-bar windows instead (manual trades, `PIVOT` mode, or a degenerate 1-bar leg).

  **Frozen impulse-leg grading for pullback entries:** the trend-strength filters — Efficiency Ratio, Bar Overlap, Break Count, Consecutive Breaks, Bar Range (incl. dominance), EMA21/EMA50 Slope, and EMA20 Gap-Bar — are meant to grade the impulse leg that led into the trade, not the pullback the entry bar sits in. For `H_SIGNAL`/`CONFLUENCE` entries (Al Brooks H/L pullback signals) they are computed over the **exact bars of the impulse leg** (from the leg's start extreme to its swing high/low — dynamic length, the configured Instrumentation Lookbacks don't apply). The leg is **frozen when the pullback starts**: H1, H2, H3… of the same pullback structure all grade the same original leg, and the leg only re-opens (metrics re-evaluate) when price breaks beyond the original swing extreme — the new leg then starts at the pullback's own deepest extreme. This holds even if the H/L count resets on a minor push high/low inside the pullback. For `PIVOT`-mode entries all windows still end at the entry bar with the configured lookbacks (no pullback concept applies). EMA20 Bias (always-in direction), ATR Depth, and the pivot-sequence filters always measure at the entry bar, since they describe current context rather than leg strength. **Note:** enabling leg grading makes lookback-tuned thresholds behave differently (e.g. Break Count `min 5` can never pass on a 4-bar leg) — re-tune thresholds against batch results.
