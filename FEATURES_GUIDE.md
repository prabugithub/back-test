...
- `legStartIndexAtEntry` / `legEndIndexAtEntry` / `legStartTimeAtEntry` / `legEndTimeAtEntry` / `legBarCountAtEntry` — the bar-index bounds (plus their candle timestamps, index-alignment-free like `LegSegment.startTime`/`endTime`) and inclusive length of the **completed breakout leg** whose bars the leg-strength metric windows covered. `undefined` means the entry was graded with entry-bar windows instead (`PIVOT` mode, or no completed leg existed yet at entry).
