...
- `efficiencyRatioAtEntry` — Kaufman Efficiency Ratio over the last N bars ending at entry: net price displacement divided by total bar-to-bar distance traveled, bounded 0–1 (near 1 = efficient one-directional trend, near 0 = chop/cancelling moves). Lookback `N` is configurable via the **Efficiency** field in the Auto-Backtest panel (default 10).
