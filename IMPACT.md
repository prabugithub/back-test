...
### Frozen impulse-leg grading + Consecutive Breaks filter (`indicators.ts`, `pivotAnalysis.ts`, `autoBacktestEngine.ts`, `useFilterPreviewData.ts`, `RegimeWorkflowSteps.tsx`)

Supersedes the trend-anchor fix above. Two problems it solved: (1) the anchored window had a **fixed lookback ending at the anchor**, so it mis-covered legs shorter/longer than the lookback; (2) `calculateAlBrooks` resets `latestHigh` when H1 fires, so **H2's anchor pointed at the minor H1→H2 push**, not the original breakout leg. Requirement (user decision): grade the *actual leg extent*, freeze it when the pullback starts, keep it for H1/H2/H3… until price breaks the original extreme, and grade *all* strength metrics on it.
...
- **`computeEntryMetrics` (`autoBacktestEngine.ts`)** — 4th param is now `legWindow?: LegWindow | null` (the `trendAnchorIndex: number` param is **gone**; TS catches stale callers). With a valid leg (`endIndex > startIndex`), ER/overlap/breaks/bar-ranges/EMA-slopes/EMA20-gap-bar/consecutive-breaks all cover exactly `[legStart, legEnd]` via derived lookbacks (`legSpan` for bar-to-prev-bar metrics, `legSpan + 1` for whole-bar sampling — verified against each function's `start =` arithmetic). EMA20 **bias** (`closeAboveRatio`) deliberately stays at `currentIndex` via a second `calculateEMAInteraction` call (gap-bar and bias now come from different windows). Null/degenerate leg or `PIVOT` mode → old behavior exactly (config lookbacks at `currentIndex`). Snapshot gained `legStartIndex`/`legEndIndex`/`legBarCount`/`maxConsecutiveHighBreaks`/`maxConsecutiveLowBreaks`; Trade records stamp all five as `...AtEntry` (no fallback recompute — `undefined` means entry-bar windows were used).
- **`calculateConsecutiveBreaks` (`pivotAnalysis.ts`)** — longest run of bars each breaking prior high without breaking prior low (mirror for lows); outside/inside bars reset the run. Gated by `passesConsecutiveBreak` + new `RegimeRules.consecutiveBreakFilter`/`consecutiveBreakThreshold` (optional → old Firestore configs load unchanged, filter off; no preset/default changes). Wired into `evalLong`/`evalShort`, `useFilterPreviewData` (`'consecutiveBreak'` key), `ConfirmationStep` UI, and `countActiveConfirmationFilters` (`StrategySummaryBar.tsx`).
- **Behavioral shift:** saved configs with ER/overlap/break/range/slope/gap-bar filters active now grade the leg instead of fixed lookbacks in H/L-signal modes — thresholds tuned to lookback windows may need re-tuning (e.g. Break Count `min 5` can never pass on a 4-bar leg).
- **Verified:** 18-assertion synthetic-candle check (H1/H2 identical frozen legs, new leg from pullback low after breakout, bear mirror via price negation, outside-bar run reset) + `npm run build` + `npm run backtest:eval` end-to-end.
