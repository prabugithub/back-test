...
→ on entry (not `isReducing`): delegates to `buildEntryInstrumentation()` in `utils/entryInstrumentation.ts` (shared verbatim with the multi-trade `openIndependentPosition` path) and spreads its `fields` onto the `Trade` — `atrDepthAtEntry`, `brrAvgAtEntry`/`brrAvgIQRAtEntry`, `ema21SlopeAtEntry`/`ema50SlopeAtEntry`, `ema20GapBarRatioAtEntry`/`ema20CloseAboveRatioAtEntry`/`ema20InteractionWindowAtEntry`, `pivotHighSeqAtEntry`/`pivotLowSeqAtEntry`/`pivotGapAvgBarsAtEntry`, `legSequenceAtEntry`. The single-leg strength and leg-bound scalars are **no longer stamped** (see "Auto-backtest regime instrumentation" below) — those metrics still run, but only into `EntryMetricsSnapshot` for filter gating. Lookbacks come from `autoBacktestConfig.barQualityLookback`/`ema21SlopeLookback`/`ema50SlopeLookback`/`emaInteractionLookback`/`legSequenceCount`/`legSequenceDetail`
...
- **Execution details popup** — primary chart only, wired via `chart.subscribeClick`. A click that lands on a bar with `time` matching one or more `trades[].timestamp` (exact match — raw `trades` timestamps are Unix seconds, same unit as candles, unlike the ms-normalized `pos.entryTime`/`exec.timestamp` copies `groupTradesIntoPositions` produces for display) opens a card at the click point for every matching execution. Body is a raw `JSON.stringify(trade, null, 2)` dump (not a curated field list) so every `Trade` field — including all surviving `*AtEntry` instrumentation and the nested `legSequenceAtEntry` segments — is directly inspectable, console-object-style. Because it dumps whatever keys the object actually has, fields removed from the `Trade` type simply stop appearing on new trades with no change needed here (old Firestore trades still show their stale keys). Gated on `activeTool === 'none'` so it never steals a click meant for placing a drawing. Local `useState`, not store state — closes on: playback `currentIndex` changing, a `mousedown` outside both the popup and the chart container, or an in-chart click that doesn't land on a trade bar.

---

## Auto-backtest regime instrumentation (`pivotAnalysis.ts`, `autoBacktestEngine.ts`)

> **Stamping vs. gating (read this first).** These helpers are called from exactly one place now: `computeEntryMetrics` in `autoBacktestEngine.ts`, which fills an `EntryMetricsSnapshot` used for **filter gating** and the config UI's live preview. They are no longer stamped onto `Trade` — the single-leg scalars (`barOverlapAtEntry`, `barOverlapAvgAtEntry`, `barRangeAvgAtEntry`, `bullBarRangeAvgAtEntry`, `bearBarRangeAvgAtEntry`, `efficiencyRatioAtEntry`, `highBreakCountAtEntry`, `lowBreakCountAtEntry`, `barBreakWindowAtEntry`, `maxConsecutiveHighBreaksAtEntry`, `maxConsecutiveLowBreaksAtEntry`) and the leg-bound scalars (`legStartIndexAtEntry`, `legEndIndexAtEntry`, `legStartTimeAtEntry`, `legEndTimeAtEntry`, `legBarCountAtEntry`) were **removed from the `Trade` type** in favor of `legSequenceAtEntry`, which carries the same leg plus the preceding N-1 with richer detail. Only `atrDepthAtEntry`, `brrAvgAtEntry`/`brrAvgIQRAtEntry`, `ema21SlopeAtEntry`/`ema50SlopeAtEntry`, `ema20*AtEntry`, `pivot*AtEntry`, `legSequenceAtEntry`, and the session-open fields (`openBarTimestampAtEntry`/`dayOpenAtEntry`/`prevDayCloseAtEntry`/`gapPointsAtEntry`/`gapPercentAtEntry`/`barsSinceOpenAtEntry` — see the "Session-open context" section below; those come from `sessionDay.ts`, not from this snapshot) are still stamped. Trades saved to Firestore before this change keep the old keys as untyped extras; nothing reads them, and no migration exists.
>
> **Do not delete any helper in this section** because its `Trade` field is gone — every one still feeds a live filter gate, and `calculateBarBreaks` additionally feeds `legSequence.ts` → `LegSegment.highBreakCount`/`lowBreakCount`, which **is** rendered in `AdvancedChart.tsx`'s leg-sequence panel.

- **`calculateBarOverlap(candles, currentIndex, lookback)`** in `utils/pivotAnalysis.ts` — rolling bar-to-bar overlap ratio (unclamped, can be negative for gapped bars) for the `lookback` bars ending at `currentIndex`. Feeds `EntryMetricsSnapshot.barOverlapAvg` → the `barOverlapFilter` gate and the Live Filter Preview.
- **`averageBarOverlap(ratios)`** in the same file — mean of the ratios array. Kept as a shared helper (not inlined at each call site) so every path computes the average identically.
- **`AutoBacktestConfig.barOverlapLookback`** (default `8`) — global setting, editable via the "Overlap" input in `SessionSettingsPanel.tsx`. Still live: it sizes the window on the **no-leg-window path** in `computeEntryMetrics` (`PIVOT` mode / no completed leg). Falls back to `?? 8` for sessions/configs saved before this field existed. Same applies to `barRangeLookback`, `efficiencyRatioLookback`, `barBreakLookback`, and `consecutiveBreakLookback`.
...
- **`calculateBarRanges(candles, currentIndex, lookback)`** / **`averageBarRanges(samples)`** — same file, same shape (raw-array function + separate average helper). Range is plain `high - low` per bar (deliberately **not** the gap-adjusted true range `calculateATR` uses — different purpose, don't conflate the two). Classifies each bar `bull`/`bear`/`neutral` (`close` vs `open`); an exact doji (`close === open`) is `neutral` — excluded from the bull/bear split but still counted in the overall average. **Known limitation:** a near-doji (tiny nonzero body, big wicks) still sorts into bull or bear by sign, so it can inflate that bucket's average — no body-ratio threshold exists yet to catch this; deliberately deferred (no real data to calibrate a threshold against), same as bar-overlap thresholding. Feeds `EntryMetricsSnapshot.barRangeAvg`/`bullBarRangeAvg`/`bearBarRangeAvg` → the `barRangeFilter` gate (incl. `dominance`); lookback is `AutoBacktestConfig.barRangeLookback` (default `20`), editable via the "Bar Range" input in `SessionSettingsPanel.tsx`.
- **`calculateBarQuality(candles, currentIndex, lookback)`** / **`averageBarQuality(samples)`** — same file, same shape as `calculateBarRanges`/`averageBarRanges` (raw-array function + separate average helper), but per-bar rather than per-window-average-only: `barQualityOf()` returns `{brr, clv, uwr, lwr}` for one candle — `brr = |close-open|/(high-low)` (Body-to-Range Ratio, trend-conviction vs. doji/spinning-top), `clv = (close-low)/(high-low)` (Close Location Value, "where the bar finished the fight" — near 1 bullish, near 0 bearish), `uwr`/`lwr = (high-max(open,close))/(high-low)` / `(min(open,close)-low)/(high-low)` (wick ratios — a large `uwr` on an up-close bar exposes a rejection-at-highs fakeout the close alone hides). All four are `0` (not `NaN`) for a zero-range bar. The per-candle values reach the `Trade` record only **per leg segment**, via `legSequenceAtEntry[].brr`/`clv`/`uwr`/`lwr` (`full` detail mode) and the always-present `brrAvg`/`clvAvg`/`uwrAvg`/`lwrAvg` — there are no flat `Trade`-level arrays for these. Both window-level BRR averages are stamped directly: `Trade.brrAvgAtEntry` (plain mean, from `averageBarQuality`) and `Trade.brrAvgIQRAtEntry` (Tukey-fence-trimmed mean, from `averageBarQualityIQR`) — same samples, same window, so they are equal when the window holds no outliers and diverge exactly when it does. Lookback (used only when no completed-leg window applies) is `AutoBacktestConfig.barQualityLookback` (default `20`), editable via the "Bar Quality" input in `SessionSettingsPanel.tsx`. **No `RegimeRules` filter/threshold exists for this metric yet** — instrumentation only, unlike the other 7 categories described below.
- **`calculateEfficiencyRatio(candles, currentIndex, lookback)`** — same file. Kaufman ER: `|close_now - close_N_bars_ago| / Σ|close_i - close_i-1|`, bounded `[0,1]`. Unlike the other two metrics this returns a single scalar directly (no raw-array counterpart — there's no meaningful "per-bar ER"). Returns `undefined` for a flat/zero-path window (would otherwise divide by zero) or when there's no prior bar at all. Complements bar overlap (adjacent-bar spatial overlap) and bar range (bull/bear bar size) by catching windows that don't overlap much locally but still cancel out net progress. Feeds `EntryMetricsSnapshot.efficiencyRatio` → the `efficiencyRatioFilter` gate (entry **and** the `exitLegDecay` re-grade); lookback is `AutoBacktestConfig.efficiencyRatioLookback` (default `10`, Kaufman's original period), editable via the "Efficiency" input in `SessionSettingsPanel.tsx`.
- **`calculateBarBreaks(candles, currentIndex, lookback)`** — same file. Counts, over the window, how many bars broke the immediately preceding bar's high (`high_i > high_{i-1}`) vs. how many broke its low (`low_i < low_{i-1}`) — independent counts, so an **outside bar counts toward both** (do not "fix" this into an either/or classification). Also returns `barsCompared`, the actual number of bar-to-bar comparisons made (`<= lookback`, shorter early in a session) — part of the helper's contract, though `computeEntryMetrics` no longer forwards it now that `barBreakWindowAtEntry` is gone. Feeds `EntryMetricsSnapshot.highBreakCount`/`lowBreakCount` → the `barBreakFilter` gate, **and** `legSequence.ts`'s `makeSegment` → `LegSegment.highBreakCount`/`lowBreakCount`, which is displayed in the chart's leg-sequence detail panel. Lookback is `AutoBacktestConfig.barBreakLookback` (default `20`), editable via the "Breaks" input in `SessionSettingsPanel.tsx`.
...
To avoid computing any metric twice per signal, `evaluateAutoSignals` computes **all 7 categories once per bar** into an `EntryMetricsSnapshot` object (not just ER as before) and returns it on `AutoSignal.entryMetrics` (this **replaced** the old single-field `AutoSignal.efficiencyRatioAtEntry`). Both `batchBacktestSimulator.ts` (`enterPosition`) and `sharedActions.ts` (`executeTrade` → `entryInstrumentation.ts`, via the `entryMetricsOverride` param — typed `EntryMetricsSnapshot`) prefer the snapshot's per-field values over recomputing, falling back to a fresh calculation per-field only for manual (non-auto-engine) trades or fields the override doesn't carry. This matters because `calculateEMASlope`/`calculateEMAInteraction` rerun `calculateEMA` over the *entire* visible candle history each call — recomputing them a second time per trade entry is real, avoidable cost in batch mode.

**`EntryMetricsSnapshot` carries only what is still read.** Fields whose sole consumer was a now-removed `Trade` stamp were pruned with them: `barBreakWindow`, `legStartIndex`, `legEndIndex`, `legStartTime`, `legEndTime`, `legBarCount`. What survives in the leg family is **`legTooShort`** — load-bearing, do not remove: it blocks auto entries when a leg-strength filter is active (`evaluateAutoSignals`, `evaluateAutoExitSignal`) and drives the Live Filter Preview (`useFilterPreviewData.ts`). Inside `computeEntryMetrics` the **local** `legBarCount` is also load-bearing — it derives `windowBars` (capped by `legMaxBarCount`), which windows every leg-strength metric, and it computes `legTooShort` against `legMinBarCount`. Removing the local, as opposed to the snapshot field, silently reverts every metric to its configured fixed lookback.

Not to be confused with the identically-named `AlBrooksMarker.legStartIndex`/`legEndIndex` in `indicators.ts` — those are the *source* of the `LegWindow` passed into `computeEntryMetrics` and are read in `evaluateAutoSignals` and `useFilterPreviewData.ts`. They were never touched by the `Trade`-field removal.
...
### Session-open context (`sessionDay.ts`, stamping sites, export)

`utils/sessionDay.ts` is the **only** day-boundary primitive in the codebase — before it there was no `firstBarOfDay`/`isNewDay`/`prevDayClose`/`sessionOpen` anywhere. Three exports, all pure `(candles, index)` with no store or config access:
- **`istDayIndex(timestamp)`** — `Math.floor((ts + 19800) / 86400)`, the IST calendar-day key. Uses the same `IST_OFFSET = 19800` as `resampler.ts` and, like it, assumes candle timestamps are **true UTC epoch seconds** (the local NIFTY JSON is corrected back with `parseColumnarData(raw, -19800)` at load). Deliberately **not** `new Date(...).getDate()` and **not** `candleTimeMinutes` from `autoBacktestActions.ts` — that one silently depends on the browser's timezone being IST. Calendar-day (rather than session-relative) bucketing is sufficient *only* because the 09:15–15:30 IST session never crosses IST midnight; if a session that wraps midnight is ever added, this key breaks.
- **`getSessionOpenContext(candles, index)`** — walks backwards from `index` while the previous bar shares its IST day. Bounded by bars-per-day (75 on 5m), so no caching; same backward-walk shape as the `pivotAnalysis.ts` primitives. Returns `undefined` only for an out-of-range index.
- **`buildSessionOpenFields(candles, index)`** — the `Partial<Trade>` slice, **omitting undefined keys entirely** (Firestore rejects explicit `undefined`).

Stamped as `Trade.openBarTimestampAtEntry` / `dayOpenAtEntry` / `prevDayCloseAtEntry` / `gapPointsAtEntry` / `gapPercentAtEntry` / `barsSinceOpenAtEntry`, from **exactly two sites, both inside their existing `!isReducing` guard** — `entryInstrumentation.ts` (spread into `fields`; covers manual + auto live-replay via `sharedActions.ts`'s `executeTrade`) and `batchBacktestSimulator.ts`'s `enterPosition` (`Object.assign(trade, …)`; **easy to miss — without it headless batch runs silently lack the fields**). Both call the one shared helper rather than hand-rolling the fields, so they cannot drift.

**Deliberately not wired into `EntryMetricsSnapshot`/`computeEntryMetrics`.** That struct exists to feed *gating* (see the stamping-vs-gating preamble above); these values are stamp-only and deterministic from `(candles, index)`, so there is no `entryMetricsOverride` path and nothing for the auto engine to supply. If a gap-based **entry gate** is ever added, it must move into the snapshot and additionally be mirrored in `useFilterPreviewData.ts` or the Live Filter Preview will disagree with the engine.

**No `AutoBacktestConfig` field, by design.** This feature has no window bound — no lookback, no min/max, no configurable N — so the "every metric window bound belongs in Session Settings, never a code literal" rule does not apply. `IST_OFFSET` is a timezone fact, not a tunable. Do not add a knob here just for symmetry with the lookback metrics.

**Check when changing:** gap fields are `undefined` (not `0`) whenever the loaded array has no previous trading day — same contract as the other `*AtEntry` metrics; `barsSinceOpenAtEntry` and the open-bar fields are still present in that case, so testing `gapPointsAtEntry !== undefined` is the correct "is there a gap reading" check, not truthiness (a genuinely flat open is `0`). Partial trading days in the candle cache (a data hole, e.g. a day whose first cached bar is 13:45) make that bar the "open bar" — expected, and consistent with `resampler.ts`, which buckets pre-09:15 bars to slot 0. On a `1D` timeframe every bar is its own day, so `barsSinceOpen` is always `0` — degenerate but correct. Firestore needs no change (flat scalars, nothing for `stripLegSequenceArrays` to touch), and the Trade History **JSON** export picks them up free via the raw `{...t}` spread; the **CSV** export carries no `*AtEntry` field at all and was left that way. Verified by `frontend/scripts/sessionGapVerify.ts` (`npx tsx scripts/sessionGapVerify.ts`), which asserts one open bar per distinct IST day, the `barsSinceOpen` step/reset, gap arithmetic, timeframe invariance across 5m/15m/60m, and that batch-simulated trades match `buildSessionOpenFields`.

### Leg-sequence context (`legSequence.ts`, `indicators.ts`, stamping sites, Firestore, export)

`Trade.legSequenceAtEntry: LegSegment[]` captures the last **N** Al Brooks impulse legs **plus the pullback candles between them**, contiguous back from the entry bar. It **replaced** the single-leg `leg*AtEntry` scalars (`legStartIndexAtEntry`/`legEndIndexAtEntry`/`legStartTimeAtEntry`/`legEndTimeAtEntry`/`legBarCountAtEntry`), which were removed once this superset existed: segment index 0's `leg` is the same graded breakout leg, carrying its `startIndex`/`endIndex`/`startTime`/`endTime`/`barCount` plus per-segment quality stats. Dependency chain:
- **`indicators.ts` — `runAlBrooks`** now also accumulates a chronological `legHistory: CompletedLeg[]` (each `{direction:'bull'|'bear', startIndex, endIndex}`), pushed at the two existing leg-freeze points, **and** a dense `signalsByBar: (AlBrooksSignal|null)[]` written at the two fire sites. **Both are purely observational** — neither may ever be read back into the H/L signal state (`hCount/hArmed/lCount/lArmed`), same invariant the file's leg-tracking comment already states.
  - The four-field result is now the exported **`AlBrooksRun`** interface, because the same shape appears in three places that must stay in lockstep: `runAlBrooks`'s return, the `alBrooksCache` WeakMap value type, and `getFullAlBrooks`'s return. **Add a field to the interface, not to three literals.**
  - **`calculateAlBrooksRun(candles)` replaced `calculateAlBrooksLegHistory`** (deleted — its only consumer was `legSequence.ts`). It returns the whole run, so a caller needing both leg history and signals pays **one** O(n) pass, not two. `calculateAlBrooks` / `calculateAlBrooksLegs` read only their own field and are unaffected; their output is byte-identical to before.
  - **`signalsByBar` is written BEFORE the pullback-depth gate**, unlike `markers`. `hCount++` already runs before that gate, so gating the array would leave a hole (`H1 → H3`) in the very sequence that exists to show the count. Consequence to remember: under `usePullbackDepth=true`, `signalsByBar[i] != null` no longer implies a marker exists at bar `i`. Nothing reads both today.
  - **One slot per bar is only sound because H and L are mutually exclusive per bar** (the outside-bar tiebreak decides by close direction). That invariant is already load-bearing for every `markers.find(m => m.time === ts)` consumer; if it is ever relaxed, `signalsByBar` silently drops a signal. `legSequenceVerify.ts` now asserts no two markers share a timestamp.
- **`utils/legSequence.ts` — `buildLegSequence(candles, currentIndex, count, detail, run?)`** — pure builder. The 5th param was **widened from `legHistory?: CompletedLeg[]` to `run?: LegSequenceSource`** (`Pick<AlBrooksRun,'legHistory'|'signalsByBar'>`) — one channel, so a caller can't hand in a leg history and a signal array computed over different arrays (indices are absolute; mismatched sources would silently misalign labels with no error). Still no caller passes it; when omitted, one `calculateAlBrooksRun(candles.slice(0, currentIndex+1))` supplies both. Sorts usable legs (endIndex ≤ entry) by start, keeps the last `count`, emits each as a `kind:'leg'` segment and fills the gaps between/after legs as `kind:'pullback'` segments via a running `cursor` (guarantees contiguity + no overlap even when the independent bull/bear machines produce time-overlapping legs), then **returns the array reversed (newest→oldest)** — index 0 is the segment closest to the entry bar. Per-segment metrics **reuse** `calculateBarQuality`/`averageBarQuality`/`calculateBarBreaks` from `pivotAnalysis.ts`; each segment also carries `startTime`/`endTime` (candle timestamps, for chart mapping) and its price `high`/`low`. Pullback `direction` = opposite of the leg it retraces.
- **Per-candle direction (`bullBear` / `bullCount`)** — computed in `makeSegment`'s existing high/low loop (no extra pass, no new `pivotAnalysis.ts` helper): `bullBear: (0|1)[]` is one entry per candle oldest→newest, `1` = bull (`close > open`), `0` = bear **or doji** (`close <= open`); `bullCount` is its sum. Applies to `leg` **and** `pullback` segments, so a leg's counter-candles and a pullback's with-trend candles are both visible. `bullBear` follows the `brr/clv/uwr/lwr` contract (**`'full'` detail only**, stripped for Firestore); `bullCount` is a required always-present field alongside `brrAvg`/`highBreakCount`, so it **survives persistence** — it's the collapsed form of the array. Note `bullCount` is absent on trades restored from sessions saved before this field existed, so UI reads must guard (the `AdvancedChart` panel uses `Number.isFinite`).
- **Per-candle Al Brooks labels (`hl` / `hlSeq`)** — same `makeSegment` loop, same contract as `bullBear`/`bullCount`: `hl: (string|null)[]` is `'full'`-detail-only and stripped for Firestore; `hlSeq: string` is required/always-present and **persists** (`''` = none fired, deliberately not `undefined` — `sanitizeData` drops `undefined`, which would make "computed, none fired" indistinguishable from "legacy trade"). `hlSeq` is exactly `hl.filter(Boolean).join('-')`, asserted in the verify script. Same legacy-restore hazard as `bullCount`: the `AdvancedChart` panel guards with `typeof s.hlSeq === 'string'` (a `=== undefined` check on a required field is a TS2367 error). Two things that look like bugs but aren't: a segment mixing H and L labels (independent counters — only the per-bar slot is exclusive), and a count restarting mid-segment (`'L1-L2-L1'` — the swing broke, so Brooks counting reset).
- **Per-candle raw OHLC (`o`/`h`/`l`/`c`)** — same `makeSegment` loop again (it already dereferences all four fields for the high/low accumulation and the bull/bear test, so this is push-only — no extra pass). Four parallel `number[]`, oldest→newest, index-aligned with `bullBear`/`hl`/`brr`; `'full'` detail only, stripped for Firestore like the others. **Unlike `bullBear`→`bullCount` and `hl`→`hlSeq`, there is deliberately NO collapsed form** — the segment's `startPrice`/`endPrice`/`high`/`low` are required always-present fields that already persist and already ARE the aggregates of these arrays, so a restored trade loses nothing but the bar-by-bar resolution. **Naming hazard:** `h`/`l` are the *per-candle* series while `high`/`low` are the *segment extremes* — they sit in the same interface and are easy to swap by accident; `legSequenceVerify.ts` pins the relationship (`Math.max(...h) === high`, `Math.min(...l) === low`, `o[0] === startPrice`, `last c === endPrice`) plus `bullBear[k] === (c[k] > o[k] ? 1 : 0)`, which is the cross-array alignment check for the whole per-candle family. No legacy-restore guard is needed in the UI beyond the existing optional chaining: the table is gated on `s.brr`, and `fmt` renders `—` for `undefined`.
- **Config:** `AutoBacktestConfig.legSequenceCount` (default 10) + `legSequenceDetail` (`'full'|'avg'`, default `'full'`), editable in `SessionSettingsPanel.tsx` ("Leg Seq N" / "Leg Seq Detail"). Both stamping sites fall back to `?? 10` / `?? 'full'` for older configs.
- **Stamping** (both, gated by `!isReducing` like the other `*AtEntry` fields): `sharedActions.ts` `executeTrade` (manual/replay, over `visibleCandlesForEntry`) and `batchBacktestSimulator.ts` `enterPosition` (over the full `candles`). Each recomputes `legHistory` internally (one extra O(n) per *entry*, not per bar — acceptable; not reused from `evaluateAutoSignals` since that returns only markers).
- **Persistence:** `firebaseSessionService.ts` `stripLegSequenceArrays` drops the per-candle `brr/clv/uwr/lwr/bullBear/hl/o/h/l/c` arrays from every segment in `saveSession`/`saveSnapshot` (averages + structure retained) — keeps trade docs under Firestore's 1MB/doc limit. **Full arrays live in-memory only**; recover them via `PerformanceDashboard.tsx` `handleExportJSON` ("Export JSON (full detail)").
- **Display:** compact leg/pullback chip strip in `AdvancedChart.tsx`'s trade-record popup (visual only); the popup's raw `JSON.stringify(t)` dump already includes it. **Clicking a chip** sets local `selectedSegment` state → `handleCustomRender` draws a translucent canvas band over the segment's candles (mapped by `startTime`/`endTime`, dashed border for pullbacks) and an effect calls `scheduleRender()` + `setVisibleLogicalRange` to pan it into view; a detail panel (move/high/low/range/breaks/bull-bear counts/**Brooks H/L** sequence/averages + per-candle table with **Dir** ▲/▼ from `bullBear` and an **H/L** column from `hl`) expands below the strip; the table also carries **O/H/L/C** columns from the per-candle price arrays (11 columns total — the container is `overflow-auto` + `whitespace-nowrap` so it scrolls inside the 26rem popup instead of wrapping; add further columns at that cost, not by shrinking the 9px font). Note the panel now has two "H/L" labels with different meanings — "H/L breaks" is `highBreakCount`/`lowBreakCount`, "Brooks H/L" is `hlSeq`; keep them distinctly named. A third meaning is now in the table header, where **H** and **L** are the bar's raw high/low. Selection auto-clears on `currentIndex` change / popup close.
- **Known inefficiency (deliberate, not yet fixed):** `buildLegSequence` recomputes on `candles.slice(0, currentIndex+1)` — a fresh array, so the `alBrooksCache` WeakMap always misses — costing one full O(n) pass **per trade entry**. `batchBacktestSimulator.ts` already passes the full `candles` array, so it could be routed through a cached `getAlBrooksRunUpTo(candles, candleIndex)` and drop to O(1): legal by causality, since the `endIndex <= currentIndex` filter already discards anything the prefix wouldn't have produced (the verify script asserts full-array and sliced builds agree). Touches the batch hot path, so it wants its own verification pass. `entryInstrumentation.ts` passes a slice and would not benefit.
- **Phase 2 (not built):** feeding `legSequenceAtEntry` into `evaluateAutoSignals`/`passes*` entry gating — high-risk (touches the entry path); requires its own impact analysis.
...
**Adding a new fixed-lookback `*AtEntry` metric (e.g. `brrAvgIQRAtEntry`)** touches, in order: (1) a pure calculator in `pivotAnalysis.ts` (e.g. `averageBarQualityIQR`, built on the existing `calculateBarQuality` samples); (2) a new optional `AutoBacktestConfig` lookback field (never a code literal — see the Session Settings rule below) + its `defaultAutoBacktestConfig` value + a new field on `EntryMetricsSnapshot`, all in `autoBacktestEngine.ts`, plus computing it inside `computeEntryMetrics` (reuses the function's existing `end`/`windowBars` leg-vs-fixed-lookback resolution, so leg-graded entries window it exactly like every other leg-strength metric); (3) `entryInstrumentation.ts`'s `fields` object (covers `executeTrade` + `openIndependentPosition` for free); (4) `batchBacktestSimulator.ts`'s separate fallback block (not covered by (3) — see above); (5) the `Trade` interface in `types/index.ts`; (6) a Session Settings input in `SessionSettingsPanel.tsx`'s "Instrumentation Lookbacks" card. `useFilterPreviewData.ts`/the Live Preview Strip need no changes to surface the raw value (it returns the whole `EntryMetricsSnapshot`), only if the metric also gains a `RegimeRules` gate.

A **second view of an existing metric** (e.g. `brrAvgAtEntry`, the plain mean beside `brrAvgIQRAtEntry`) skips (2)'s new config field and (6) entirely: it must reuse the original metric's samples and lookback (`barQualityLookback` here) so the two remain directly comparable — a second lookback for the same underlying window would make the pair meaningless. Everything else in the list still applies.
...
- **`computeEntryMetrics` (`autoBacktestEngine.ts`)** — with a leg window: `windowBars = min(legBarCount, legMaxBarCount ?? 15)`; ER/overlap/bar-breaks get `windowBars - 1` (pair-comparison conventions reach one bar back, so this keeps windows inside the leg; a 1-bar leg yields `undefined` metrics), bar-ranges/EMA20-interaction/bar-quality get `windowBars`, consecutive-breaks gets `[end - windowBars + 1, end]`. EMA21/EMA50 slopes deliberately keep config lookbacks (leg-end anchored). Snapshot gained `legTooShort` (`legBarCount < legMinBarCount ?? 5`). New config fields `legMinBarCount`/`legMaxBarCount` (top-level, optional → old Firestore configs load with 5/15 defaults), editable in Session Settings → Instrumentation Lookbacks. Bar quality (`calculateBarQuality`, added later — see the instrumentation section above) plugs into this same `windowBars ?? config.barQualityLookback` pattern. Note `legBarCount` and `legTooShort` remain here as a **local** and a snapshot field respectively even though the `legBarCountAtEntry` stamp was removed — see the `EntryMetricsSnapshot` note in the instrumentation section for why neither can go.
...
## `TradeHistoryDialog.tsx` — modal → full-page conversion

`TradeHistoryDialog` was converted from a floating, draggable modal (`fixed inset-0` backdrop + centered box, click-outside-to-close) to a **full-page overlay** (`absolute inset-0 z-[105]`), matching the pattern already used by `PerformanceDashboard` (`z-[110]`) and `AutoBacktestPanel` (`z-[110]`). This is a **presentation-only change** — the CSV/JSON export handlers, `groupTradesIntoPositions`/`calculatePerformanceStats` usage, and the jump-to-candle logic (`scrollToTime`/`highlightCandle`) are untouched. (Props were subsequently renamed again — see the `PageNavTabs` section below — `onClose`/`onOpenDashboard` no longer exist; it's `onNavigate` now.)

- **Removed:** all drag state (`offset`, `isDragging`, `dragStart`, `startOffset`, `handleMouseDown`) and the click-outside-to-close `useEffect` — full-page views don't drag or dismiss on outside click.
- **Z-index convention:** full-page overlays in this app render as siblings inside the same `relative`-positioned container in `App.tsx` (`<div className="flex-1 flex flex-col relative h-full">`) and stack purely by `z-*` value, not DOM order. `TradeHistoryDialog` is `z-[105]` — deliberately *below* `PerformanceDashboard`/`AutoBacktestPanel` (`z-[110]`) so that navigating from Trade History to the Dashboard/Backtest layers them on top rather than being hidden behind it. **Check when adding a new full-page overlay:** pick a `z-*` relative to this existing stack (105 < 110 < 115/120 for `SessionSettingsPanel`'s drawer < 200 for `PromptDialog`/`ConfirmDialog`), don't reuse `z-[2000]` (still used by the floating-modal-style `TradeReportDialog`, which is unrelated dead code — not imported anywhere).

---

## `PageNavTabs.tsx` (new) — cross-page navigation + mount-once/hide-don't-unmount state persistence

Follow-up to the `TradeHistoryDialog` full-page conversion above: the four full-page views (Chart, Trade Log, Backtest, Dashboard) previously had ad-hoc, one-directional navigation (each dialog's own boolean state in `App.tsx` or, for `AutoBacktestPanel`, local state buried in `PlaybackControls`; "Dashboard" was reachable from Trade History but Backtest wasn't reachable from either). This introduced a single `ActivePage` union (`'chart' | 'tradeLog' | 'backtest' | 'dashboard'`) and a shared `PageNavTabs` pill-switcher component so every page is reachable directly from every other page's header, and so a page's internal state (scroll position, expanded rows, filters, search text, sub-tabs) survives navigating away and back instead of resetting.

- **New single source of truth in `App.tsx`:** `activePage: ActivePage` (which page is showing) and `visitedPages: Set<ActivePage>` (which pages have been opened at least once, always includes `'chart'`). `navigateTo(page)` sets both. The old per-dialog booleans (`isTradeHistoryOpen`, `isPerformanceDashboardOpen`) and `PlaybackControls`' local `showAutoBTPanel` are gone.
- **Mount-once, hide-don't-unmount:** `TradeHistoryDialog`/`PerformanceDashboard` are only rendered once `visitedPages.has(page)` is true, then stay mounted — a wrapping `<div style={{ display: activePage === page ? undefined : 'none' }}>` toggles visibility instead of the old `{isOpen && <Component/>}` conditional-mount pattern. **This is the mechanism that now preserves scroll position / expanded rows / filters / search text / sub-tab selection across navigation** — do not revert to `&&`-gated mounting for these two, that reintroduces "state resets every time you reopen it."
- **`AutoBacktestPanel` is a `createPortal(..., document.body)`** — a hiding wrapper `<div>` around it in `App.tsx` has **zero effect**, because the portaled subtree renders as a child of `document.body`, not of that wrapper. It's mounted unconditionally once `visitedPages.has('backtest')`, and visibility is controlled via a `hidden?: boolean` **prop on the component itself**, applied to its own root `<div>`'s `style={{ display: hidden ? 'none' : undefined }}`. **Check when adding another portal-based full-page view:** it needs the same `hidden` prop pattern, not a wrapping div — the two are not interchangeable and the mistake fails silently (no error, the component just never actually hides).
- **Prop rename across all three full-page components:** `TradeHistoryDialog`, `PerformanceDashboard`, `AutoBacktestPanel` each replaced their bespoke close/back prop (`onClose`, `onBack`, `onClose` respectively) with a single `onNavigate: (page: ActivePage) => void`, wired to `<PageNavTabs active="<ownPageKey>" onNavigate={onNavigate} />` in their header (replacing the old "Back to Chart" button and, for `TradeHistoryDialog`, the separate purple "Dashboard" button). `TradeHistoryDialog`'s jump-to-candle handler now calls `onNavigate('chart')` instead of `onClose()`.
- **`PlaybackControls` prop changes:** gained `onOpenBacktest?: () => void` (wired to `navigateTo('backtest')` in `App.tsx`), lost the local `showAutoBTPanel` state and its `AutoBacktestPanel` import/render — that component now lives only in `App.tsx`. The Zap/"Auto Backtesting Engine" toolbar button calls `onOpenBacktest?.()` unconditionally (no more click-to-toggle-closed; closing now happens via navigating to a different page).
- **Ambiguous-selector gotcha for anyone scripting/testing this UI:** because every visited page keeps its own copy of `PageNavTabs` mounted (hidden) in the DOM, a plain `querySelector`/`getByText` for a nav button label (e.g. "Dashboard") can resolve multiple elements — the visible one plus every hidden page's copy. CSS `:has-text`-style ancestor matching also matches hidden descendants' `textContent`, so div-scoping by heading text doesn't disambiguate either. The chart page's own bottom-controls-bar `PageNavTabs` is a further trap: it's never `display:none` (only ever covered by a higher-z-index full-page overlay), so a plain visibility check on it can report "visible" even while a real user's click would land on whatever's actually on top.
...
## Per-array memoization cache for pivots/EMA/ATR/AlBrooks (`indicators.ts`, `pivotAnalysis.ts`, `autoBacktestEngine.ts`)

**Why:** the auto-backtest engine (`evaluateAutoSignals`, `computeEntryMetrics`, `evaluateTrailStop`, `evaluateAutoExitSignal`) and `sharedActions.ts`'s `checkTrendReversal` were each re-deriving `calculatePivotPoints`/`calculateAlBrooks`/`calculateEMA`/`calculateATR` from scratch on **every bar** by calling them on a freshly-sliced `candles.slice(0, currentIndex + 1)` — up to several times per bar (once per regime tried, once per open position in multi-trade mode, once each for trail-stop/exit-signal checks). Since those functions scan the whole prefix, that's O(n) work repeated for every one of n bars → O(n²) total, which is what made auto-backtest (interactive auto-advance and the batch simulator) slow on any sizeable candle history. Fixed in this pass — 6.3x faster on a headless `npm run backtest:eval` run (5037-candle dataset), byte-identical trade output verified via before/after diff, plus `exitEngineSmoke.ts` (27/27) and `legSequenceVerify.ts` still pass.

- **New cache layer in `indicators.ts`:** `WeakMap`s keyed on the exact `candles` array reference, storing the full-array result of `calculatePivotPoints`/`calculateEMA`/`calculateATR`/`runAlBrooks` computed once. New exported readers — `getPivotPointsUpTo`, `getEmaValueAt`, `getAtrValueAt`, `getAlBrooksMarkersUpTo`, `getAlBrooksLegsAt` — take the FULL candles array + an index and do an O(1)/O(log n) lookup instead of re-slicing + recomputing.
- **Correctness invariant this relies on:** `calculatePivotPoints`, `calculateEMA`, `calculateATR`, and `runAlBrooks` (`calculateAlBrooks`/`calculateAlBrooksLegs`/`calculateAlBrooksLegHistory`) must stay **strictly causal** — the value at bar `i` may only depend on `candles[0..i]`, never anything after it. That's what makes "compute once on the full array, read off index i" identical to "recompute on `candles.slice(0, i+1)` every time". **If you ever add a lookahead to any of these (e.g. a repaint-style pivot confirmation that peeks forward), the cache in `indicators.ts` will silently serve stale/wrong values — audit it first.**
- **Cache scope:** only hits when the SAME array reference is reused across calls (batch simulator and the interactive per-bar store actions both hold `candles` steady across a run, so this is exactly the hot path). A fresh `.slice()` or a different array always misses and just recomputes — nothing regresses, it only fails to speed up.
- **`autoBacktestEngine.ts` call sites updated:** `evaluateAutoSignals`, `computeEntryMetrics`, `evaluateTrailStop`, `evaluateAutoExitSignal`, `resolveExitRules`, `getCurrentMarketState`, `getEmaAt`/`getAtrAt` (now thin wrappers over the cached lookups — used elsewhere too, e.g. `useFilterPreviewData.ts`, which gets the speedup for free).
- **New `analyzeMarketStructureAt(candles, currentIndex, pivots)` in `pivotAnalysis.ts`** — same regime-detection logic as `analyzeMarketStructure(candles, pivots)`, but takes the full array + an index instead of a pre-sliced view, so it can use `getEmaValueAt` instead of recomputing `calculateEMA` over the whole prefix. The original `analyzeMarketStructure` is untouched and still used by its other (non-hot-loop) callers — manual-trade analysis (`analyzePivotForTrade`/`analyzeManualEntry`), `entryInstrumentation.ts`, `ChartToolbar.tsx`, `executeTrade`'s manual path in `sharedActions.ts` — since those run once per user action/render, not once per bar.
- **`sharedActions.ts`'s `checkTrendReversal`** (runs every bar of `step()` while a position is open) and **`PlaybackControls.tsx`'s `memoizedPivots`** were switched to the cached lookups too, since both re-derive on every bar during auto-advance.
- **Deliberately left un-cached:** `buildEntryInstrumentation` (`entryInstrumentation.ts`) — only runs once per trade entry, not per bar, so its plain `calculatePivotPoints`/`calculateEMA`/`calculateATR`/`calculateAlBrooksLegs` calls on a slice are not a hot path. `AdvancedChart.tsx`/`ChartToolbar.tsx`/`TradeJournalDialog.tsx` also left as-is (visual rendering, lower risk area, not the reported bottleneck).
