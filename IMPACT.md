...
**Efficiency ratio as an entry gate (not just instrumentation):** `RegimeRules.efficiencyRatioFilter` (`'none' | 'min' | 'max'`, default `'none'`) + `efficiencyRatioThreshold` (default `0.3`) let a regime require ER ≥/≤ threshold before firing — same `'none'`/`min`/`max` idiom as `atrDepthFilter`, gated via `passesEfficiencyRatio()` in `autoBacktestEngine.ts` right after the existing `passesAtrDepth` check in `evalLong`/`evalShort`. To avoid computing ER twice per signal, `evaluateAutoSignals` computes it once and returns it on `AutoSignal.efficiencyRatioAtEntry`; both `batchBacktestSimulator.ts` (`enterPosition`) and `sharedActions.ts` (`executeTrade` via the `entryMetricsOverride` param) prefer that value over recomputing, falling back to a fresh `calculateEfficiencyRatio` call only for manual (non-auto-engine) trades. Designed so bar-overlap/EMA-slope/gap-bar-ratio filters can be added the same way later without restructuring `RegimeRules` or the eval functions again.

### `AutoBacktestConfig` / `RegimeRules` shape (`autoBacktestEngine.ts`)

Previously undocumented here. Global config (`AutoBacktestConfig`): `enabled`, `skipIfPositionOpen`, `tradeStartTime`/`tradeEndTime`, `useAutoQty` (default **`true`**)/`riskPerTrade`/`minQuantity` (risk-based position sizing — already fully wired in both `batchBacktestSimulator.ts` and `autoBacktestActions.ts`'s `runAutoBacktestCheck`, identical formula: `qty = floor(riskPerTrade / |entry - sl|)`, skip if `< minQuantity`), `autoSquareOff`/`squareOffTime`, `slTpFillMode` (see `checkSLTPHits` entry above), and the 7 instrumentation lookback fields (see above). Per-regime (`RegimeRules`, one each for `uptrend`/`downtrend`/`range`/`reversal`): `enabled`, `direction`, `entryMode` (`PIVOT`/`H_SIGNAL`/`CONFLUENCE`), `allowH1/H2/L1/L2`, `confluenceLookback`, `ltPivotSequence`, `maFilter`, `atrDepthFilter?`/`atrDepthThreshold?`, `efficiencyRatioFilter?`/`efficiencyRatioThreshold?`, `htStructureFilter`, `slMethod`/`slAtrMultiplier`/`slFixedPoints`, `targetRR`. `AUTO_BT_PRESETS`' `applyPreset` (`AutoBacktestPanel.tsx`) explicitly re-applies the *current* session's `useAutoQty`/`riskPerTrade`/`minQuantity` after spreading a preset — so the `useAutoQty` default only affects brand-new sessions, never an existing session's or preset's choice.
