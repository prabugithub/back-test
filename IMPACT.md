...
- **Persistence:** `firebaseSessionService.ts` `stripLegSequenceArrays` drops the per-candle `brr/clv/uwr/lwr` arrays from every segment in `saveSession`/`saveSnapshot` (averages + structure retained) — keeps trade docs under Firestore's 1MB/doc limit. **Full arrays live in-memory only**; recover them via `PerformanceDashboard.tsx` `handleExportJSON` ("Export JSON (full detail)").
- **Display:** compact leg/pullback chip strip in `AdvancedChart.tsx`'s trade-record popup (visual only); the popup's raw `JSON.stringify(t)` dump already includes it. **Clicking a chip** sets local `selectedSegment` state → `handleCustomRender` draws a translucent canvas band over the segment's candles (mapped by `startTime`/`endTime`, dashed border for pullbacks) and an effect calls `scheduleRender()` + `setVisibleLogicalRange` to pan it into view; a detail panel (move/high/low/range/breaks/averages + per-candle table) expands below the strip. Selection auto-clears on `currentIndex` change / popup close.
