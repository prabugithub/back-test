...
`Trade.legSequenceAtEntry: LegSegment[]` captures the last **N** Al Brooks impulse legs **plus the pullback candles between them**, contiguous back from the entry bar — the multi-leg counterpart to the single-leg `leg*AtEntry` fields. Dependency chain:
