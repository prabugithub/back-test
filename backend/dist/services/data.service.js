 ...
 */
function generateSimCandles(securityId, intervalMinutes) {
    ...
    try {
        ...
        if (!isDhanAvailable) {
            if (IS_SIM) {
                ...
                logger_1.default.info(`[SIM] No Dhan credentials — serving ${synthetic.length} synthetic candles for ${params.securityId}`);
                return synthetic;
            }
            logger_1.default.error('Dhan credentials missing in .env');
            throw new Error('Dhan credentials missing in .env');
        }
        // 2. Fetch ONLY from Dhan API
        logger_1.default.info('MODE: DHAN EXCLUSIVE - Fetching candles exclusively from Dhan API', {
        ...
        try {
            const candles = await (0, dhan_service_1.fetchHistoricalCandles)({
                securityId: params.securityId,
            ...
            if (candles && candles.length > 0) {
                logger_1.default.info(`Successfully fetched ${candles.length} candles from Dhan`);
            ...
            logger_1.default.warn('Dhan API returned no data for this request');
            return [];
        }
        catch (err) {
            logger_1.default.error('Dhan exclusive fetch failed:', err.message);
            if (IS_SIM) {
                const intervalMinutes = parseInt(params.interval, 10) || 5;
                const synthetic = generateSimCandles(params.securityId, intervalMinutes);
                logger_1.default.warn(`[SIM] Dhan fetch failed — serving ${synthetic.length} synthetic candles for ${params.securityId}`);
                return synthetic;
            }
            throw err;
        }
        ...
        logger_1.default.error('Failed to get candles (Exclusive Mode):', error.message);
        throw error;
    }
}
/**
...
function clearCache(params) {
    logger_1.default.info('Cache clearing disabled in EXCLUSIVE mode');
}
