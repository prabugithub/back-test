...
export interface VirtualOrder {
    ...
    createTime: string;
    updateTime: string;
    // Simulation control (not part of real Dhan response)
    _sim_autoFillDelayMs: number;
    _sim_partialFillQty: number | null;
    _sim_shouldReject: boolean;
    _sim_fillPrice: number | null;
}
...
interface DefaultFillBehavior {
    autoFillDelayMs: number;
    rejectNextOrder: boolean;
...
class VirtualOrderBookImpl {
    ...
    private fillTimers = new Map<string, NodeJS.Timeout>();

    private defaultBehavior: DefaultFillBehavior = {
        autoFillDelayMs: 0,
        rejectNextOrder: false,
        partialFillQty: null,
    };
    private nextOrderOverride: Partial<VirtualOrder> | null = null;

    private generateOrderId(): string {
        return `SIM-ORD-${Date.now()}-${this.orderCounter++}`;
    }

    setDefaultFillBehavior(b: Partial<DefaultFillBehavior>): void {
        this.defaultBehavior = { ...this.defaultBehavior, ...b };
    }

    setNextOrderOverride(o: Partial<VirtualOrder>): void {
        this.nextOrderOverride = o;
    }

    placeOrder(params: PlaceOrderParams): VirtualOrder {
        ...
        logger.info(`[VirtualOrderBook] Placed order ${orderId} ${params.transactionType} ${params.quantity}x ${params.securityId} @ ${params.price || 'MKT'}`);
    ...
    getOrder(orderId: string): VirtualOrder | undefined {
    ...
    getAllOrders(): VirtualOrder[] {
        return Array.from(this.orders.values()).map(o => {
            ...
            return clean as VirtualOrder;
        });
    }

    fillOrder(orderId: string, fillQty: number, fillPrice: number): void {
        const order = this.orders.get(orderId);
        if (!order || order.orderStatus === 'REJECTED' || order.orderStatus === 'CANCELLED') return;
        if (order.orderStatus === 'TRADED') return;
        ...
        logger.info(`[VirtualOrderBook] Filled order ${orderId} qty:${actualFillQty} @ ${order.tradedPrice} → ${order.orderStatus}`);
    ...
    rejectOrder(orderId: string, reason: string): void {
        ...
        order.updateTime = new Date().toISOString();
        logger.info(`[VirtualOrderBook] Rejected order ${orderId}: ${reason}`);
    }

    modifyOrder(orderId: string, patch: { orderType?: 'MARKET' | 'LIMIT'; price?: number; quantity?: number; exchangeSegment?: string }): VirtualOrder | null {
        const order = this.orders.get(orderId);
        if (!order) return null;
        if (order.orderStatus !== 'PENDING' && order.orderStatus !== 'TRANSIT') return null;

        const wasLimit = order.orderType === 'LIMIT';
        if (patch.orderType) order.orderType = patch.orderType;
        ...
        logger.info(`[VirtualOrderBook] Modified order ${orderId} → type:${order.orderType} price:${order.price}`);
    ...
    private updatePosition(order: VirtualOrder, fillQty: number, fillPrice: number): void {
    ...
    updatePositionPrice(securityId: string, price: number): void {
    ...
    reset(): void {
        ...
        this.nextOrderOverride = null;
        logger.info('[VirtualOrderBook] Reset — all orders and positions cleared');
    }
}

export const virtualOrderBook = new VirtualOrderBookImpl();
